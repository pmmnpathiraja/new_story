...
class RegisterValidation extends StatefulWidget {
  RegisterValidation(this.buyingData);
...
class _RegisterValidationState extends State<RegisterValidation> {
  ...
  Future<void> validation() async {
      ...
      DocumentSnapshot docCurrent = await usersref.doc("Farmer_Details").get();

      if (docCurrent.exists) {
        widget.buyingData.farmerName = docCurrent.data()['Farmer Name'];
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) =>
                    purchaseReceipt(widget.buyingData, _farcontact.text)));
      } else {
        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) =>
                    registerFarmer(widget.buyingData, _farcontact.text)));
      }
  }
